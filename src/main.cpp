...
uint256 hashGenesisBlock("0x00000c3d630550c09692bf7da1a894d38a3e03ba2fc31256692a80ac4f8f9b36"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TwilightCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TwilightCoin Signed Message:\n";
...
{
    // TwilightCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // TwilightCoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // TwiLightCoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TwilightCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, twilightcoin - DarkGravity, written by Evan Duffield - evan@darkcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave2(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, twilightcoin - DarkGravity v2, written by Evan Duffield - evan@darkcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TwilightCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // TwilightCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000020a16b9bd647d36d53f686042ea5b30230d5ed64d41e8b8ad8ffbc08f46");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Time 06/Apr/2014 Protesters clashed with police in Donetsk, Luhansk and Kharkiv, waving Russian flags and calling for independence from Ukraine";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1396839385;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 28958929;

        if (fTestNet)
        {
            block.nTime    = 1396839385;
            block.nNonce   = 3861857969;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa37425bb5d80c39f346725c3a635ecfd0163909a7d05dccc0e5712a3a7189fa0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // TwilightCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TwilightCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TwilightCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TwilightCoinMiner(CWallet *pwallet)
{
    printf("TwilightCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("twilightcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TwilightCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TwilightCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TwilightCoinMiner, pwallet));
}
